...
const AllTransactionTable = () => {
  ...
  const fetchCommissionEarnings = useCallback(async () => {
    ...
    try {
      ...
      apiRequest({
        endpoint: COMMISSION_ALL_TRANS_HISTORY,
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        data: requestData,
      }).then((response) => {
        if (response?.data) {
          setEarnings(response.data.tranactions || []); // Note: API has typo 'tranactions'
  ...
  }, [selectedType, fetchCommissionEarnings]);

  // Clear filter effect
  useEffect(() => {
    if (isFiltering && !startDate && !endDate && !selectedType) {
      fetchCommissionEarnings();
    }
  }, [startDate, endDate, selectedType, isFiltering, fetchCommissionEarnings]);
              ...
              {paginatedData.length > 0 ? (
                paginatedData.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-6 w-32">
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-1 rounded text-sm font-medium border whitespace-nowrap ${getTypeColors(
                          item.type
                        )}`}
                      >
